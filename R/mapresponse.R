...
#' 
#' @description Map response variables using [contour()], [image()], and
#' [filled.contour()].
#' 
#' @details
#' [contour()], [image()], and [filled.contour()] are powerful and do the
...
#' correct format.
#' 
#' The first, [coldata.to.imagemat()], converts a vector of response variables 
#' and converts it into a matrix of the correct form for use by [contour()], 
#' [image()], and [filled.contour()]. There must be one value of the response 
#' `z` for every position in a grid overlain on the map. In the case of CTFS 
#' Plots, this means a value for `z` at every quadrat, though quadrat can be any
#' size.  The function [abundanceperquad()] produces exactly the correct kind of
#' vector for use by [coldata.to.imagemat()]. The function [gxgy.to.index()] 
...
#' 
#' In that case, the length of vector `z` must be exactly 1250, `length(x)` must
#' be exactly 50, and `length(y)` exactly 25. Alternatively, x and y can be set
#' 
#' NULL and it will be calculated from `plotmax = c(1000, 500)` and `plotmin =
#' c(0, 0)`. The simplest application of this function for CTFS plots is to set
#' x and y NULL, and `plotmax` to the correct plot dimensions. Then make sure
#' `z` is the right size for the number of quadrats.
#' 
#' The common error with image and contour maps is getting the proper dimensions
#' for `z` and the proper sizes for `plotmax`, `x`, and `y`. If you get an error
...
#' *  `x`: same as the argument x; if x is submitted as NULL, this is the vector
...
#' *  `mat`: The matrix needed for graphing, exactly as needed for instance by
#' [imageGraph()].
#' 
#'
#' @template gridsize_map
#' @param z A numeric vector of responses, which means an attribute which has
#'   map coordinates.
...
#' @param plotmin The minimum plot x and y coordinates; in CTFS plots, these are
#'   always 0,0. Ignored if x and y are submitted.
#' @param plotmax Maximum plot x and y coordinates. The default, 1000,500, is
