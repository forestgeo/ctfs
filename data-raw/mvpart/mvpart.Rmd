...
```
The clustering actually is weird. The first division is OK, but not the
second. It groups the top of the hill and the bottom of the hill (red and
green on the map) together in the second split. The species are really
different between these habitats, so I am wondering if there is an error.  It
looks to me like the analysis just grouped on soils, and did not consider
species data.
```
...
#### Was KC3spp20 already a matrix?

__No (in terms of R).__

> KC3spp20 was already a matrix (of species * quadrat).

R does not interpret KC3spp20 as a matrix:

```{r}
KC3spp20[1:6, 1:6] %>% is.matrix()
data.matrix(KC3spp20)[1:6, 1:6] %>% is.matrix()
```



#### What does `data.matrix()` do?

__Converts a dataframe to a matrix.__

> What did your command [`data.matrix(KC3spp20)`] do to the species data?
...
```{r}
# First few rows and columns of each
KC3spp20[1:6, 1:6]
data.matrix(KC3spp20[1:6, 1:6])
```
...
```{r, error=TRUE}
abundance_raw <- KC3spp20
formula_raw <- abundance_raw ~ RB_NO3 + RB_NH4 + RB_PO4 + Al + pH_water + Na + 
  Mn + Mg + K + Fe + Ca + BS + ECEC + Bray_P + meanelev + convex + slope
mvpart_run_raw <- mvpart(form = formula_raw, data = environmental_variables)
```
...
...
The classification tree seems to be entirely driven by the soils, and the species don’t carry much weight. Perhaps standardizing the abundances or something would help? 
 
My map of the 8 clusters is fine. It represents distinctly different habitats. But, the order of classification in the tree is weird.
...
- Are soils values much higher than those of other environmental variables?
