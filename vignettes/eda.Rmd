...
Some functions we will use are available in the __[forestr](https://forestgeo.github.io/forestr/)__ package, and many are available in the __tidyverse__ package (which includes, for example, the packages __[dplyr](http://dplyr.tidyverse.org/)__ and __[ggplot2](http://ggplot2.tidyverse.org/)__). If you are new to the __tidyversre__, you may feel a little frustrated at the beginning. But quickly your effort will pay-off. The tools in the __tidyverse__ are powerful, relatively easy to use and consistent; so once you learn some, you will learn most other tools intuitively.

To more completely learn how to do data science with the __tidyverse__ read [R for Data Science](http://r4ds.had.co.nz/), by Garrett Grolemund and Hadley Wickham.
...
Frequently I refer to functions from a particular package as `package::fun()` (e.g. the first time I use a function). Although the prefix `package::` is unnecessary if the package has previously been loaded with `library(package)`, I will often keep that prefix to be explicit about where the function comes from. This should help you to find the function if you ever want to use it again.
...
If you want to handle the data faster and print it nicer, convert data frames to tibbles:
...
Although the [data dictionary](https://goo.gl/wGKEE3) does not explicitly refer to `A`, `D` and `M`, you likely know or guess that they mean alive, dead and missing. To confirm this, let's first count the the number of unique values of the variables `status` and `DFstatus` (`DFstatus` is not described in the [data dictionary](https://goo.gl/wGKEE3)), then joining both counts by the shared variable `n`:
...
```{r, fig.asp = 1, fig.align="default", out.width = "50%", message = FALSE}
ggplot(data = large_dataset) +
...
```{r odd-agb, echo=FALSE}
ggplot(mapping = aes(x = dbh, y = agb)) + 
  geom_point(data = large_dataset) +
  geom_point(data = large_dataset %>% filter(dbh > 80 & dbh < 140),
    color = "red"
  )
||||||| merged common ancestors
...
large_dataset2 <- large_dataset %>% 
  add_residuals(mod) %>% 
  mutate(resid = exp(resid))

also_strange <- large_dataset2 %>% filter(dbh < 80, resid > 5)

ggplot(mapping = aes(x = dbh, y = resid)) + 
  geom_point(data = large_dataset2) +
  geom_point(data = large_dataset2 %>% filter(dbh > 80 & dbh < 140),
    color = "red"
  ) +
  geom_point(data = also_strange, size = 3, colour = "red", shape = 1)
```
