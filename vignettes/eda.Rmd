...
subtitle: "Adapted for [ForestGEO](https://twitter.com/ForestGEO?lang=en) from [R for Data Science](http://r4ds.had.co.nz/), by [Garrett Grolemund](https://twitter.com/StatGarrett) and [Hadley Wickham](https://twitter.com/hadleywickham)"
author: "Mauro Lepore"
date: "`r Sys.Date()`"
output: 
...
This vignette literaly reproduces the section _Exploratory Data Analysis_ in [R for Data Science](http://r4ds.had.co.nz/), by [Garrett Grolemund](https://twitter.com/StatGarrett) and [Hadley Wickham](https://twitter.com/hadleywickham), except that the examples and some text were changed to more strongly engage ecologysts from ForestGEO.
...
By default tibbles print few rows; to print more rows use:
...
```{r, out.width="30%", fig.align="default"}
# Use the DRY principle (Don't Repeat Yourself)
hist_vary_binwidth <- function(binwidth) {
  ggplot(data = small_dbh, mapping  = aes(x = dbh)) +
    geom_histogram(binwidth = binwidth)
}
binwidths_to_try <- c(10, 30, 60)
purrr::map(.x = binwidths_to_try, .f = hist_vary_binwidth)
```
...
```{r hist-vs-freqpoly-compare, fig.cap="For multiple histograms, lines are easier to understand than bars.", out.width="45%", fig.align="default"}
...
p <- ggplot(data = small_cut_number, aes(x = dbh, color = equal_n)) +
  # place legend inside plot area (defults is outside)
  theme(
...
In the next plot, however, the clustering may be an artifact of the chosen bar-width; the clustering disappears with bars twice as wide.
...
```{r, fig.align="default", out.width="45%"}
# Left plot; don't remove NAs and get a warning on the console
unusual_only %>% 
  ggplot(aes(dbh)) + 
    geom_histogram(binwidth = useful_barwidth) +
    labs(title = "Expect empty plot but get a warning on the console")
...
p <- small_dataset %>% ggplot(aes(sp, dbh))
p + geom_point()
p + geom_jitter()
...
large_dataset <- bci12full7 %>% 
  filter(dbh < 400) %>% 
  sample_n(100000)
p <- large_dataset %>% ggplot(aes(dbh, agb))
p
```
...
```{r, fig.asp = 1, fig.align="default", out.width = "45%", message = FALSE}
# Left
p + geom_bin2d()
...
```{r model-agb, dev = "png", out.width="45%", fig.align="default"}
library(modelr)
