...
- first, before any complex summary or model, analyse how interesting variables vary, here, species (categorical) and growth between intervals (continuous).

xxx explain why use the pipe. clear writing comes from position (Goepen xxx). Ideas are clearly communicated if information flows from new old to old, and agents are mapped to subjects, and actions to verbs (Style by Strunk or white?). 
This structure is impossible with traditional programming
...
# Install from the tidyverse http://tidyverse.org/
library(dplyr)
library(purrr)
library(tibble)
library(modelr)
