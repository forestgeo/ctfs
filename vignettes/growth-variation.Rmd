...
xxxcont. Use data from several censuses in bci, then let the chunk below run

```{r xxxLoadData, include=FALSE}
censusdata <- c("bci12full1.rda", "bci12full7.rda")
map2("../bci/data/", censusdata, paste0) %>% 
  walk(load, .GlobalEnv)
```

```{r, eval=FALSE}
# See defaults and get help with ?individual_grow.table

# Choose censuses you wish to work with
censuses <- list(bci12full1, bci12full7)
# Choose tree diameter between mindbh and maxdbh (here using big trees)
grate <- individual_grow.table(censuses, mindbh = 400, maxdbh = 10000)
